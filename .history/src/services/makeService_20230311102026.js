...
class MakeService {
  ...
  getFilteredMakes = (name) => {
    return axios
      .get(`https://localhost:44304/api/FilteredVehicleMakes?name=${name}`)
      .catch(errorHandler);
  };

  getPagedMakes = (pageNumber, pageSize) => {
    return axios
    .get(`https://localhost:44304/api/PagedVehicleMakes?queryParameters.pageNumber=${pageNumber}&queryParameters.pageSize=${pageSize}`)
    .catch(errorHandler);
  };
